...
function print_help {
    ...
    echo "ssh port: $PORT"
    echo 
    echo "Connect your container directly via \`ssh root@<host-ip> -p $PORT\`."
...
function do_start {
    ...
    if [ $? -ne 0 ]; then
        ...
        sleep 3
        echo ""
        echo "please set your password when you login"
    else
...
function do_run {
    if [ ! -L $HOME/root-in-container ];then
        ln -s /var/lib/lxd/storage-pools/zfs-pool/containers/$USER/rootfs/root $HOME/root-in-container
    fi
    lxc exec $USER bash
}
